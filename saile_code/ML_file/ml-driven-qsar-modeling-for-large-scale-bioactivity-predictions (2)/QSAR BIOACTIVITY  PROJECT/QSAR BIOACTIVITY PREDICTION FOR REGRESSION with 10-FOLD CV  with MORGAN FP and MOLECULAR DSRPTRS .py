...
# Morgan fingerprints
x_morgan = smile_to_morganprint(data_clean["Smiles"],radius=2,n_Bits=1024)

x_morgan = pd.DataFrame(x_morgan,columns=[f"FP_{i}" for i in range(1024)],index=data_clean.index)
...
for smi in data_clean["Smiles"]:
    ...
    values = [func(mol) if mol else np.nan for name, func in Descriptors._descList]
...
descriptor_df = pd.DataFrame(descriptor_data,columns=descriptor_names,index=data_clean.index)


# Remove invalid columns
descriptor_df = descriptor_df.replace([np.inf, -np.inf],np.nan).dropna(axis=1)


# Remove low variance
selector = VarianceThreshold(0.05)

descriptor_df = pd.DataFrame(selector.fit_transform(descriptor_df),columns=descriptor_df.columns[selector.get_support()],index=data_clean.index)


# Scale descriptors
scaler = StandardScaler()

descriptor_scaled = pd.DataFrame(scaler.fit_transform(descriptor_df),columns=descriptor_df.columns,index=data_clean.index)


# Combine Morgan + descriptors
X = pd.concat([x_morgan, descriptor_scaled],axis=1)
...
#### this below code for  single run code
